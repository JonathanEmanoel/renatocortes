...
import { CalendarDays, Clock, MapPin, MessageSquare, Scissors, User, X } from "lucide-react";
    ...
    <>
      ...
      {isDetailsOpen ? (
        <div className="fixed inset-0 z-[70] grid place-items-end bg-black/78 px-4 py-4 backdrop-blur-sm md:place-items-center">
          <section className="w-full max-w-xl overflow-hidden rounded-[8px] border border-white/16 bg-[#070707] shadow-panel">
            <div className="flex items-center justify-between border-b border-white/10 p-5">
              <div>
                <p className="text-xs font-bold uppercase tracking-[0.22em] text-primary">Detalhes do agendamento</p>
                <h2 className="mt-2 text-xl font-black uppercase">Reserva #{appointment.id.padStart(4, "0")}</h2>
              </div>
            ...
            <div className="grid aspect-[16/6] place-items-center bg-gradient-to-br from-[#1b120d] via-black to-[#090909]">
              <Scissors className="h-14 w-14 text-primary" />
            </div>
            ...
            <div className="p-5">
              <div className="mb-6 flex items-center justify-between gap-4">
                <Badge tone={appointment.status === "Confirmado" ? "green" : "white"}>{appointment.status}</Badge>
                <span className="text-sm text-white/52">Dados fictícios para integração futura</span>
              </div>
              ...
              <div className="grid gap-3">
                ...
                <DetailRow icon={MapPin} label="Unidade" value="Renato Cortes Barbearia - Unidade Principal" />
                <DetailRow icon={MessageSquare} label="Observação" value="Chegar 10 minutos antes do horário marcado." />
              </div>
